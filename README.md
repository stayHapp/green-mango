...
第一版聚焦最小可用闭环：管理员创建会议并配置报名字段，用户访问会议页面并提交报名，管理员查看报名记录。
...
- 报名字段配置：每个会议可配置自己的报名字段
- 字段类型：`text`、`number`、`select`
- 字段属性：字段名称、字段标识、是否必填、排序、选项配置
- 前台报名：根据会议字段配置动态生成报名表并提交
- 后台报名管理：查看某个会议的报名记录和字段值

第一版暂不做：微信登录、支付、短信验证码、邮件通知、复杂权限、复杂统计、文件上传、报名审核、Excel 导出、多租户和高级页面装修。
...
4. [MVP 范围](docs/product/mvp-scope.md)：第一版做什么和暂不做什么
