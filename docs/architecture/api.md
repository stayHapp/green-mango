...
签到记录列表响应在基础字段之外携带嘉宾身份与同行绑定信息：`guest_name`、`guest_phone`、`is_companion`（是否同行嘉宾）、`companion_of_id`（所属主嘉宾 ID，普通嘉宾为 `null`）、`companion_of_name`（所属主嘉宾姓名）与 `companion_note`（随行备注，如家属、司机）。工作人员端据此在签到记录中展示主嘉宾「已带 N 人」徽标、同行人员「同行 · 随某嘉宾」标签与随行备注，并按 `companion_of_id` 将同行签到归入主嘉宾记录下的展开子列表；主嘉宾未签到但同行已签到时，该同行记录作为独立记录行保留。
