...
未提供手机号的嘉宾导入时自动填充默认手机号（后端配置 `default_guest_phone`，默认 `13900010001`），保证手机号字段非空且登录可核验；同会议内重名且都无手机号的嘉宾会因身份唯一约束无法同时导入，需由管理员调整其中一个的手机号。导出 Excel 中默认手机号显示为「未提供」，避免误当成真实号码。
